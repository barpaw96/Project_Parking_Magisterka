...
while True:
    ...
    #plt.show()
        ...
        lenrowAD = 0
        if VW == "Volkswagen":
            sql_VW = "SELECT row_number() over (order by id), name, idcolor_car, idvehicle_brand, number_plates FROM users WHERE idvehicle_brand = %s AND idcolor_car =%s"
            cursor = connection.cursor()
        ...
        if AD == "Audi":
            sql_AD = "SELECT row_number() over (order by id), name, idcolor_car, idvehicle_brand, number_plates FROM users WHERE idvehicle_brand = %s AND idcolor_car =%s"
            cursor = connection.cursor()
            ...
            for rowAD in cursor:
                ...
                lenrowAD = rowAD[0]
        if TY == "Toyota":
            sql_TY = "SELECT row_number() over (order by id), name, idcolor_car, idvehicle_brand, number_plates FROM users WHERE idvehicle_brand = %s AND idcolor_car =%s"
            cursor = connection.cursor()
        ...
        if HY == "Hyundai":
            ...
            for rowHY in cursor:
                ...
                lenrowHY = rowHY[0]
        if REN == "Renault":
            sql_REN = "SELECT row_number() over (order by id), name, idcolor_car, idvehicle_brand, number_plates FROM users WHERE idvehicle_brand = %s AND idcolor_car =%s"
            cursor = connection.cursor()
